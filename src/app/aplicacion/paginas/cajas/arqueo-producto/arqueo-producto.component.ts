...
@Component({
  selector: 'app-arqueo-producto',
  templateUrl: './arqueo-producto.component.html',
  styleUrls: ['./arqueo-producto.component.scss'],
  providers: [DatePipe]
})
export class ArqueoProductoComponent implements OnInit {
  ...
  // OBTENER ARQUEOS PRO PRODUCTO
  obtenerArqueosPorProducto(): void {
    this.cargando.show();
    this.cajaServicio.obtenerArqueoPorProducto({ idArqueo: this.idArqueo }).subscribe((respuesta: any) => {
      console.log('REPUESTA DE ARQUEO POR PRODUCTO: ', respuesta.lista);
  ...
  obtenerTotal(): void {
    ...
    this.productos.forEach((fila: any) => {
      this.totalProductos = this.totalProductos + fila.venta.subTotal;
    });
  }
  ...
  cargar(id: any): void {
    ...
    this.obtenerArqueosPorProducto();
  }
  ...
  cambiar(): void {
    ...
    if (this.registroFormGroup.invalid) {
      ...
      this.router.navigate(['cajas/arqueo/producto/' + this.r.idArqueo.value]);
